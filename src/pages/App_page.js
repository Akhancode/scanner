    ...
    //  GET COINGECKO DATA
      ...
      coingecko.map((coin,index)=>{
        ...
        FilteredData.push({
          ...
          "ETH_USDT":ETH_USDT,
          "BTC_USDT":BTC_USDT,
          "profit_ETH":Math.round((((100/(Binancedata?.filter((x)=>x.symbol === `${coinSymbol}USDT`))[0]?.price)*(Binancedata?.filter((x)=>x.symbol === `${coinSymbol}ETH`))[0]?.price*ETH_USDT)-100)*1000)/1000,
          "profit_BTC":Math.round((((100/(Binancedata?.filter((x)=>x.symbol === `${coinSymbol}USDT`))[0]?.price)*(Binancedata?.filter((x)=>x.symbol === `${coinSymbol}BTC`))[0]?.price*BTC_USDT)-100)*1000)/1000
  ...
  return (
    <div className="App">
      ...
      <body className="container">
        ...
        <Link to={`/top10`} style={{ textDecoration: 'none' ,color:"black"}}>
            <button>
                Top10 - Market
            </button>
        </Link>
                  ...
                  showData?.map((coin,i)=>{                  
                      {
                        ...
                        <td style={{color:((coin.profit_BTC>0.25?"lightgreen":null))}}><Link to={`/detail`} state={{ symbol:coin.symbol,pair:"BTC",curProfit:coin.profit_BTC}} className='btn_none'>{coin.profit_BTC}</Link></td>
                        <td style={{color:((coin.profit_ETH>0.25?"lightgreen":null))}}><Link  to={`/live`} state={{ symbol:coin.symbol,pair:"ETH"}} className='btn_none'>{coin.profit_ETH}</Link></td>
